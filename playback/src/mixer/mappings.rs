...
pub trait MappedCtrl {
    fn map(&self, volume: u16) -> f32;
    fn unmap(&self, mapped_volume: f32) -> u16;

    fn db_range(&self) -> u8;
    fn set_db_range(&mut self, new_db_range: u8);
...
impl MappedCtrl for VolumeCtrl {
    fn map(&self, volume: u16) -> f32 {
        ...
        let mapped_volume = if self.range_ok() {
            match *self {
                Self::Cubic(db_range) => CubicMapping::map(normalized_volume, db_range),
                Self::Log(db_range) => LogMapping::map(normalized_volume, db_range),
                _ => normalized_volume,
            }
        } else {
    ...
    fn unmap(&self, mapped_volume: f32) -> u16 {
        ...
        let unmapped_volume = if self.range_ok() {
            match *self {
                Self::Cubic(db_range) => CubicMapping::unmap(mapped_volume, db_range),
                Self::Log(db_range) => LogMapping::unmap(mapped_volume, db_range),
                _ => mapped_volume,
            }
        } else {
            // Ensure not to return -inf or NaN due to division by zero.
    ...
    fn range_ok(&self) -> bool {
        self.db_range() > 0 || matches!(self, Self::Fixed | Self::Linear)
    }
}

trait VolumeMapping {
    fn map(unmapped_volume: f32, db_range: u8) -> f32;
    fn unmap(mapped_volume: f32, db_range: u8) -> f32;
}
...
impl VolumeMapping for LogMapping {
    fn map(normalized_volume: f32, db_range: u8) -> f32 {
...
impl LogMapping {
    fn coefficients(db_range: u8) -> (f32, f32) {
        let db_ratio = db_to_ratio(db_range.into());
...
impl CubicMapping {
    fn min_norm(db_range: u8) -> f32 {
        f32::powf(
            10.0,
            -1.0 * db_range as f32 / VolumeCtrl::DEFAULT_DB_RANGE as f32,
        )
    }
}
